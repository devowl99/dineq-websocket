...
import com.dineq.dineqbe.util.TransactionUtils;
...
public class OrderService {
    ...
    public OrderStatusUpdateResponseDTO acceptOrders(List<Long> orderIds) {
        ...
        if (!toSave.isEmpty()) {
            ...
            TransactionUtils.afterCommit(() -> invalidateSender.sendAlert("ORDERS_ACCEPTED"));
        }
    ...
    public OrderStatusUpdateResponseDTO completeOrders(List<Long> orderIds) {
        ...
        if (!toSave.isEmpty()) {
            tableOrderRepository.saveAll(toSave);
            TransactionUtils.afterCommit(() -> invalidateSender.sendAlert("ORDERS_COMPLETED"));
        }
    ...
    public void cancelOneOrder(Long orderId) {
        ...
        TransactionUtils.afterCommit(() -> invalidateSender.sendAlert("ORDER_CANCELLED_ONE"));
    }

    // groupNum에 해당하는 주문내역 지우기
    @Transactional
    public void cancelAll(String groupNum) {
        if (groupNum == null) throw new IllegalArgumentException("groupNum이 비어있음");

        List<TableOrderEntity> list = tableOrderRepository.findAllByGroupNum(groupNum);
        if (list.isEmpty()) throw new IllegalArgumentException(groupNum + " groupNum에 해당하는 주문내역이 존재하지 않음");

        tableOrderRepository.deleteAll(list);
        TransactionUtils.afterCommit(() -> invalidateSender.sendAlert("ORDER_CANCELLED_ALL"));
    }
}
